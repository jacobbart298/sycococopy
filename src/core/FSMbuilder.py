...
# The FDMBuilder class is responsible to build the FSM based on a parse tree produced from a specification
...
class FSMbuilder(PythonicVisitor):
    ...
    def visitSequence(self, ctx:PythonicParser.SequenceContext) -> None:
        ...
        # check whether last child is a repeat expression and if so, 
        # takes the state to which needs to be looped from the loop_dictionary
        ...
        for index in expressionIndices:
            counter += 1
            if counter == expressionCount: #final expression reached
                if repeat:
                    nextState = loopState #return to the loopstate
                else:
                    nextState = ctx.endState #proceed to the endstate
            else:
                nextState = State() #proceed to next state in sequence
    ...
    def visitShuffle(self, ctx: PythonicParser.ShuffleContext) -> None:
        expressionCount: int = ctx.getChildCount() - 2
        expressionIndices: range = range(1, expressionCount + 1)
        # uses a permutations function to determine all possible orders in the shuffle
        for indicesPermutation in permutations(expressionIndices):
            counter: int = 0
            currentState: State = ctx.startState
            for index in indicesPermutation:
                counter += 1
                if counter == expressionCount: # last expression in shuffle reached
    ...
    # Builds the send part of an FSM, with three options: regular send, 
    # and predicate send with or without comparator. Adds the sender and receiver to the used roles set
    def visitSend(self, ctx: PythonicParser.SendContext) -> None:
        ...
        # build transition send with an equal comparator
            ...
            type: str = ctx.getChild(1).getText()
            comparator: str = "=="
            value: str = ctx.getChild(3).getText()
    ...
    # transform a string to a primitive value based on given type
    def stringToValue(self, type: str, value: str) -> any:
        match type:
            case "int":
                return int(value)
            case "float":
